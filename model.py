...
def predict_image(pil_image, model):
    """
    Expects a PIL Image object and an already-loaded ultralytics YOLO
    model. The model is owned and lazy-loaded by main.py's
    get_defect_model() -- not by this module -- so it can be unloaded
    when the garment detector needs the memory instead (only one YOLO
    model is ever resident at a time).
    """
    try:
